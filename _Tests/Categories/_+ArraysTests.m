...
//  self.equal(_(IA(1, 2, 3)).first(), 1, @"can perform OO-style "first()"");
...
//  self.equal(iResult, 4, @"works on an arguments object.");
  A* result = _.map(OA(IA(1,2,3),IA(1,2,3)), _.firstIterator); // CHANGED
  self.equal(result.join(@","), @"1,1", @"works well with _.map");
  result = (^() { return _.take(IA(1,2,3), N.i(2)); })();
...
- (void)test_initial
{
  self.equal(_.initial(IA(1,2,3,4,5), nil).join(@", "), @"1, 2, 3, 4", @"working initial()");
  self.equal(_.initial(IA(1,2,3,4), N.i(2)).join(@", "), @"1, 2", @"initial can take an index");
//  A* result = (^(){ return _(arguments).initial(); })(1, 2, 3, 4);
//  self.equal(result.join(@", "), @"1, 2, 3", @"initial works on arguments object");
  A* result = _.map(OA(IA(1,2,3),IA(1,2,3)), _.initialIterator); // CHANGED
  self.equal(_.flatten(result, nil).join(@","), @"1,2,1,2", @"initial works with _.map");
}

- (void)test_last
{
  self.equal(_.last(IA(1,2,3), nil), N.i(3), @"can pull out the last element of an array");
  self.equal(((A*)_.last(IA(1,2,3), N.i(0))).join(@", "), @"", @"can pass an index to last");
  self.equal(((A*)_.last(IA(1,2,3), N.i(2))).join(@", "), @"2, 3", @"can pass an index to last");
  self.equal(((A*)_.last(IA(1,2,3), N.i(5))).join(@", "), @"1, 2, 3", @"can pass an index to last");
//  NSInteger iResult = (^(){ return _(arguments).last(); })(1, 2, 3, 4);
//  self.equal(iResult, 4, @"works on an arguments object");
  A* result = _.map(OA(IA(1,2,3),IA(1,2,3)), _.lastIterator); // CHANGED
...
//  self.equal(result.join(@", "), @"2, 3, 4", @"aliased as tail and works on arguments object");
  A* result = _.map(OA(IA(1,2,3),IA(1,2,3)), _.restIterator); // CHANGED
  self.equal(_.flatten(result, nil).join(@","), @"2,3,2,3", @"works well with _.map");
}

//- (void)test_compact
//{
//  self.equal(_.compact(IA(0, 1, false, 2, false, 3)).length, 3, @"can trim out all falsy values");
...
//{
//  if (window.JSON) {
//    A* list = IA(1, IA(2), IA(3, IA(IA(IA(4)))));
//    self.equal(JSON.stringify(_.flatten(list)), @"IA(1,2,3,4)", @"can flatten nested arrays");
//    self.equal(JSON.stringify(_.flatten(list, true)), @"IA(1,2,3,IA(IA(IA(4))))", @"can shallowly flatten nested arrays");
//    A* result = (^(){ return _.flatten(arguments); })(1, IA(2), IA(3, IA(IA(IA(4)))));
//    self.equal(JSON.stringify(result), @"IA(1,2,3,4)", @"works on an arguments object");
//  }
//}
//
//- (void)test_without
//{
//  A* list = IA(1, 2, 1, 0, 3, 1, 4);
...
//  ok(_.without(list, listIA(0)).length == 1, @"ditto.");
//}
//
//- (void)test_uniq
//{
//  A* list = IA(1, 2, 1, 3, 1, 4);
...
//
//  list = IA({name:"moe"}, {name:"curly"}, {name:"larry"}, {name:"curly"});
...
//{
//  A* stooges = IA("moe", @"curly", @"larry"); A* leaders = IA("moe", @"groucho");
...
//- (void)test_zipObject
//{
//  O* result = _.zipObject(IA("moe", @"larry", @"curly"), IA(30, 40, 50));
...
//{
//  A* numbers = IA(1, 0, 1, 0, 0, 1, 0, 0, 0);
