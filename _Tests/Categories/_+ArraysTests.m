...
- (void)test_first
{
  ...
  equalI(__(AI(1,2,3)).first(/* REQUIRED */ -1).I, 1, @"can perform OO-style 'first()'");
...
- (void)test_initial
{
  ...
  equal(_.flatten(result, /* REQUIRED */ YES).join(@","), @"1,2,1,2", @"initial works with _.map");
}
...
- (void)test_rest
{
  ...
  equal(_.flatten(result, /* REQUIRED */  YES).join(@","), @"2,3,2,3", @"works well with _.map");
}

- (void)test_compact
{
  equalI(_.compact(AO(N.I(0), N.I(1), N.B(NO), N.I(2), N.B(NO), N.I(3))).length, 3, @"can trim out all falsy values");
...
- (void)test_flatten
{
  if (SS.JSON) {
    ...
    equal(JSON.stringify(_.flatten(list, /* REQUIRED */ NO)), @"[1,2,3,4]", @"can flatten nested arrays");
    equal(JSON.stringify(_.flatten(list, /* REQUIRED */ YES)), @"[1,2,3,[[[4]]]]", @"can shallowly flatten nested arrays");
...
- (void)test_uniq
{
  ...
  list = AI(1, 1, 1, 2, 2, 3);
  equal(_.uniqAdvanced(list, YES, /* REQUIRED */ nil).join(@", "), @"1, 2, 3", @"can find the unique values of a sorted array faster"); /* SPECIALIZED */
  ...
  equal(_.map(/* SPECIALIZED */ _.uniqAdvanced(list, NO, iterator), iterator).join(@", "), @"moe, curly, larry", @"can find the unique values of an array using a custom iterator");

  iterator = ^(N* value, ... /* KEY, COLLECTION */) { return N.I(value.I + 1); };
  list = AI(1, 2, 2, 3, 4, 4);
  equal(_.uniqAdvanced(list, YES, iterator).join(@", "), @"1, 2, 3, 4", @"iterator works with sorted array");
...
//  numbers.indexOf = null; /* NOT SUPPORTED: JavaScript-only because of native function */
  equalI(_.indexOf(numbers, N.I(2)), 1, @"can compute indexOf, even without the native function");
  ...
  equalI(_.indexOf(nil, N.I(2)), -1, @"handles nulls properly");
...
//  numbers.lastIndexOf = null; /* NOT SUPPORTED: JavaScript-only because of native function */
  equalI(_.lastIndexOf(numbers, N.I(1)), 5, @"can compute lastIndexOf, even without the native function");
  equalI(_.lastIndexOf(numbers, N.I(0)), 8, @"lastIndexOf the other element");
