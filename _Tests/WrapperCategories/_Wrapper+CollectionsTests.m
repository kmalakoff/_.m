...
//  self.equal(tripled.join(@", "), @"3, 6, 9", @"tripled numbers with context");
  ...
  doubled =  _.chain(AI(1, 2, 3)).map(^(N* num, ... /* KEY, COLLECTION */){ return N.I(num.I * 2); }).valueA();
...
//  self.equal(sum, 18, @"can reduce with a context object");
  ...
  sum =  _.chain(AI(1, 2, 3)).reduce(^(N* sum, N* num, ... /* KEY, COLLECTION */){ return N.I(sum.I + num.I); }, 0).valueN();
...
- (void)test_include
{
  ...
  self.ok(_.chain(AI(1,2,3)).include(N.I(2)).valueB(), @"OO-style include");
}
