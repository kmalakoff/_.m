...
//  equal(_.reduce(A.new, ^{}, undefined), undefined, @"undefined can be passed as a special case");
  raises(^{ _.reduce(A.new, ^(/* REQUIRED */ N* memo, N* num, ... /* KEY, COLLECTION */){ return memo; }, /* REQUIRED */ 0); }, @"TypeError", @"throws an error for empty arrays with no initial value");
}

- (void)test_reduceRight
{
  S* list = _.reduceRight(AO(@"foo", @"bar", @"baz"), ^(S* memo, S* str, ... /* KEY, COLLECTION */){ return memo.add(str); }, S.new);
  equal(list, @"bazbarfoo", @"can perform right folds");

  list = _.foldr(AO(@"foo", @"bar", @"baz"), ^(S* memo, S* str, ... /* KEY, COLLECTION */){ return memo.add(str); }, S.new);
  equal(list, @"bazbarfoo", @"aliased as 'foldr'");

  list = _.foldr(AO(@"foo", @"bar", @"baz"), ^(S* memo, S* str, ... /* KEY, COLLECTION */){ return memo.add(str); }, /* REQUIRED */ nil);
...
//  equal(_.reduceRight(A.new, ^{}, undefined), undefined, @"undefined can be passed as a special case");
  raises(^{ _.reduceRight(A.new, ^(/* REQUIRED */ N* memo, N* num, ... /* KEY, COLLECTION */){ return memo; }, /* REQUIRED */ 0); }, @"TypeError", @"throws an error for empty arrays with no initial value");
}
...
- (void)test_all
{
  ...
  ok(_.all(AI(0, 10, 28), ^B(N* num, ... /* KEY, COLLECTION */){ return num.I % 2 == 0; }), @"even numbers");
  ok(!_.all(AI(0, 11, 28), ^B(N* num, ... /* KEY, COLLECTION */){ return num.I % 2 == 0; }), @"an odd number");
...
- (void)test_any
{
  ...
  ok(!_.any(AI(1, 11, 29), ^B(N* num, ... /* KEY, COLLECTION */){ return num.I % 2 == 0; }), @"all odd numbers");
  ok(_.any(AI(1, 10, 29), ^B(N* num, ... /* KEY, COLLECTION */){ return num.I % 2 == 0; }), @"an even number");
